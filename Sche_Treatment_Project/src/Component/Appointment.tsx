...
export const AppointmentDetail: React.FC<appointmentDetailsProps> = ({
  ...
  const fetchClinic = async () => {
    try {
      ...
      setClinic(data);
      if (data.medicalAreaId) {
        const idArea = data.medicalAreaId.id;
        fetchArea(idArea);
      }
    } catch (e: any) {
      setError(true);
  ...
  const fetchArea = async (id: string) => {
    try {
      if (id || id !== null) {
        const response = await fetch(API_ENDPOINTS.GET_AREA(id), headerAuth());
            ...
            <div className="button-wrapper w90">
              ...
              <div className="table-head">
                ...
                <div className="table-center">
                  ...
                  <p className="">
                    Giới tính:{" "}
                    {bill?.appointment.appointmentGender ? "Nữ" : "Nam"}
                  </p>
                  <p className="">
                    BHYT:{" "}
